...
| `src/parse.rs` | Per-format parsers all producing `Table { headers: Vec<String>, rows: Vec<Vec<String>> }`. CSV parser: hand-rolled state machine handling quoted fields and escaped quotes. TSV: split on tabs. Markdown: strip pipes, skip separator lines. Box-drawing: strip `│`/`|` delimiters, skip border lines. ASCII-aligned: two-pass column detection — (1) 2+ space gaps in header validated against data rows (60% threshold), (2) if fewer columns than header words, gutter approach finds positions where ALL lines have a space and detects gutter→non-gutter transitions as column boundaries (handles lsof-style 1-space separated output). Last column extends to EOL. Whitespace: split on whitespace runs. First row always becomes headers. |
...
- **ASCII-aligned detection** uses a two-pass algorithm: (1) header gaps of 2+ spaces validated against data rows (60% threshold), then (2) gutter approach for 1-space gaps — finds positions where ALL lines have a space, uses gutter→non-gutter transitions as column boundaries. Pass 2 triggers when pass 1 finds fewer columns than header words. This handles both wide-gap output (kubectl, ps) and tight-gap output (lsof)
